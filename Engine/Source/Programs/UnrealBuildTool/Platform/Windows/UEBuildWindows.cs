...
#pragma warning restore CS1591
	...
	class WindowsPlatform : UEBuildPlatform
	{
...
#if true   //@LGPNOPDB for 5.4 Suppress PDB files and all Debug info                                                             
			//@LGPNOPDB for 5.4 now uses <bUsePDBFiles>true</bUsePDBFiles> in BuildConfiguration.xml 
			if (Target.bUsePDBFiles)
			{
				Target.bDisableDebugInfoForGeneratedCode = false; //@LGPNOPDB for 5.4 Suppress PDB files

			}
			else
			{
				Target.bDisableDebugInfoForGeneratedCode = true; //@LGPNOPDB for 5.4 allow PDB files

			}

#else      //false uses code below                              
		...
		public override void SetUpConfigurationEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment GlobalCompileEnvironment, LinkEnvironment GlobalLinkEnvironment)
		{
			...
			//       data completely.
			//GlobalLinkEnvironment.bCreateDebugInfo = true; //@LGPNOPDB for 5.4 Suppress uses code below
			if (Target.bUsePDBFiles)
			{
				GlobalLinkEnvironment.bCreateDebugInfo = false; //@LGPNOPDB for 5.4 Suppress PDB files

			}
			else
			{
				GlobalLinkEnvironment.bCreateDebugInfo = true; //@LGPNOPDB for 5.4 allow PDB files

			} //@LGPNOPDB END for 5.4 Suppress above code
		}
