...
,	TasksInProgressThatWillNeedHelp(0)
	// LGPU 5.3 add line 	NumTexelsNeedToBeProcessedOnGPU(0)
...
void FStaticLightingSystem::MultithreadProcess()
{
	...
	// @LGPU 5.3 BEGIN add export
	ExportAggregateMeshToGPUSolver();

	ExportSkyCubemapToGPUSolver();

	ExportPunctualLightsToGPUSolver();

	GenerateMapsForGPUSolver();

	GSwarm->SendMessage(NSwarm::FTimingMessage(NSwarm::PROGSTATE_Preparing4, 0));

	SolveRadiosityOnGPU(GeneralSettings.NumIndirectLightingBounces);
	// @LGPU 5.3 END add export
	...
	// @LGPU 5.3 Remove line for GPU 5.0.0
...
#if USE_LOCAL_SWARM_INTERFACE
		//@LGPU 5.3 remove line below for LGPU 5.0.0
...
void FStaticLightingSystem::ThreadLoop(bool bIsMainThread, int32 ThreadIndex, FThreadStatistics& ThreadStatistics, float& IdleTime)
{
	...
	while (!bIsDone)
	{
		...
		// Process any existing local tasks before fetching new tasks from Swarm
		{
			...
			//@LGPU 5.3 BEGIN add while block
			while (!TextureMappingsNeedPostprocessing.IsEmpty())
			{
				FStaticLightingTextureMapping* MappingNeedsPostprocessing = TextureMappingsNeedPostprocessing.Pop();
				if (MappingNeedsPostprocessing) // No lock here, need to check again
				{
					PostprocessTextureMapping(MappingNeedsPostprocessing);

					bAnyTaskProcessedByThisThread = true;
				}
			}
			//@LGPU 5.3 END add while block
		}
		...
		{
			//@LGPU 5.3 BEGIN add CalculateIndirectLightingTextureMappingGPU
			if (!bAnyTaskProcessedByThisThread)
			{
				CalculateIndirectLightingTextureMappingGPU(nullptr, true);
			}
			//@LGPU 5.3 END add CalculateIndirectLightingTextureMappingGPU
