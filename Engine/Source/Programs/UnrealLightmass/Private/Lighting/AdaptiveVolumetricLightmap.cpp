...
void FStaticLightingSystem::ProcessVolumetricLightmapBrickTask(FVolumetricLightmapBrickTaskDescription* Task)
{
	...
	const double StartFinalGatherTime = FPlatformTime::Seconds();

	TArray<FFinalGatherSample3> PrecomputedUpperSamples;
	TArray<FFinalGatherSample3> PrecomputedLowerSamples;
	TArray<float> PrecomputedMinDistances;
	TArray<float> PrecomputedBackfacingHitsFraction;
	PrecomputedUpperSamples.AddZeroed(BrickSize * BrickSize * BrickSize);
	PrecomputedLowerSamples.AddZeroed(BrickSize * BrickSize * BrickSize);
	PrecomputedMinDistances.AddZeroed(BrickSize * BrickSize * BrickSize);
	PrecomputedBackfacingHitsFraction.AddZeroed(BrickSize * BrickSize * BrickSize);

	CalculateVolumetricLightmapBrickSamplesGPU(
		BrickSize,
		WorldBrickMin,
		WorldChildCellSize,
		PrecomputedUpperSamples,
		PrecomputedLowerSamples,
		PrecomputedMinDistances,
		PrecomputedBackfacingHitsFraction);


	const double EndFinalGatherTime = FPlatformTime::Seconds();
	Task->MappingContext.Stats.VolumetricLightmapFinalGatherTime += EndFinalGatherTime - StartFinalGatherTime;
		...
		for (int32 Y = 0; Y < BrickSize; Y++)
		{
			for (int32 X = 0; X < BrickSize; X++)
			{
				...
				CalculateVolumeSampleIncidentRadiance(
					...
					RandomStream,
					Task->MappingContext,
					PrecomputedUpperSamples[Z * BrickSize * BrickSize + Y * BrickSize + X],
					PrecomputedLowerSamples[Z * BrickSize * BrickSize + Y * BrickSize + X],
					true,
					bDebugSamples);
				...
				//@LGPU 5.3 add 5.1.1 LGPU code MinDistanceToSurface , BackfacingHitsFraction
				BackfacingHitsFraction = PrecomputedBackfacingHitsFraction[Z * BrickSize * BrickSize + Y * BrickSize + X];
				MinDistanceToSurface = PrecomputedMinDistances[Z * BrickSize * BrickSize + Y * BrickSize + X];
